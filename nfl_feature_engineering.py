...
"""
numerical_feat = averaged_feat_22 + ["fantasy_points_ppr"]
# get averaged numerical stats by position
def get_pos_features(df, feat=numerical_feat):
    by_pos = {}
    for position in ["QB", "WR", "TE", "RB", "FB"]:
        by_pos[position] = df.loc[df.position==position, feat]
    return by_pos["QB"], by_pos["WR"], by_pos["TE"], by_pos["RB"], by_pos["FB"]

qb_23, wr_23, te_23, rb_23, fb_23 = get_pos_features(all_23)
"""
...
# among RBs with similar volume, those with high efficiency (yards per carry) will score more 
def rb_ratios(df):
    ratio_features = []
